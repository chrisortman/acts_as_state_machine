    ...
    module StateMachine              #:nodoc:
      ...
      module ActMacro
        ...
        def acts_as_state_machine(opts)
          ...
          scope(:in_state, lambda { |*states|
              states.map!(&:to_s)
              sql_string, values = [], []
              states.each { |state| sql_string << "state = ?"; values << state }
              {:conditions => [sql_string.join(" or ")] | values}
          }) do
          ...
          scope(:not_in_state, lambda { |*states|
              states.map!(&:to_s)
              sql_group = "(" + states.map{|state| "'#{state}'"}.join(',') + ")"
              {:conditions => "state NOT IN #{sql_group}"}
          }) do
            def in_state(*args)
              raise "Can't use 'in_state' named_scope with 'not_in_state' named scope"
            end
      ...
      module InstanceMethods
        def set_initial_state #:nodoc:
          write_attribute self.class.state_column, self.class.initial_state.to_s if self.state.nil?
        end
      ...
      module ClassMethods
        ...
        def event(event, opts={}, &block)
          ...
          define_method("#{event.to_s}!") do
             ...
             @transition = nil
             self.state = self.changes['state'].first unless is_fired
             is_fired
          end
        end
